...
from .tasks import send_weekly_email  # นำเข้า task ที่สร้างไว้
...
def listview(request):
    ...
    total_expired_count = 0  # เพิ่ม total_expired_count
    ...
    for country in countries:
        ...
        expired_count = certification_numbers.filter(certification__country=country, status='expired').count()  # เพิ่ม expired_count
        ...
        total_expired_count += expired_count  # เพิ่ม total_expired_count
    ...
    context = {
        ...
        'certificate_numbers_expired': total_expired_count,  # เพิ่ม total_expired_count
    }
...
def send_manual_email(request, country_id):
    ...
    return redirect('country_list')  # เปลี่ยน 'country_list' เป็น URL ที่คุณต้องการ redirect ไป
